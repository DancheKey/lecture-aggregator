...
4. 输出严格 JSON（不要 markdown 代码块、不要解释），结构如下：
{
  ...
  "topic": {"value": "单场讲座题目，无则 null；题目必须是源页的中文题，不得翻译成英文或用英文论文题替换", "snippet": "..."},
  ...
  "speakerAffiliation": {"value": "主讲人当前单位/院系，无则 null；若 speakerBio/abstract 中明确出现『XX大学XX学院』、『现任/现为 XX大学』等当前所属机构，必须提取出来；单位取到机构层级（如『厦门大学』）即可，不要擅自细化到院系/研究中心/系（源页只写『厦门大学』就不要扩成『厦门大学经济学院』）", "snippet": "..."},
  "lectureStart": {"value": "ISO8601 如 2026-08-21 15:00，未知 null", "snippet": "..."},
  "lectureEnd": {"value": "ISO8601，未知 null", "snippet": "..."},
  "location": {"value": "地点（精确到楼栋房号；线上讲座须保留会议平台与会议号，如『腾讯会议 362-688-563』，不得省略会议号；不含校名但须保留校区/学院前缀如『大学城』『石牌』）", "snippet": "..."},
...
_VERDICT_FIELD_SEMANTICS = (
    ...
    "- 采纳倾向（防大模型过度改写，2026-09-24 裁决实证）：\n"
    "  · location：宁可保留含会议号/校区前缀的规则值，也不要采纳更短且丢了"
    "『腾讯会议 362-688-563』会议号或『大学城/石牌』校区前缀的 LLM 值；"
    "仅当 LLM 值更完整（补全上下文）或只去除尾部噪声（人名/注/开幕式）时才更准。\n"
    "  · affiliation：规则已是合法机构（如『厦门大学』）时，不要采纳 LLM 在其后"
    "追加的院系/研究中心/系（如『厦门大学经济学院』）——那是过度细化；"
    "仅当 LLM 值更短（去掉脏尾巴如『/亚洲开发银行经济学家』→『亚洲开发银行』）"
    "或规则值本身错误/误赋（如把姓名当单位）时才采纳。\n"
    "  · topic：不要把源页本就正确的中文题替换成英文论文题；"
    "LLM 提炼真中文题、去掉英文括号或长尾是正确的。\n"
