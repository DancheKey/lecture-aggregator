...
def _extract_meeting_info(raw):
    ...
    s = str(raw)
    plat = ''
    if re.search(r'腾\s*讯\s*会\s*议|企业微信|腾讯会议（企业版）|腾讯会议\(企业版\)', s):
        plat = '腾讯会议'
    ...
    if m:
        ...
        if m:
            ...
            m = re.search(r'腾\s*讯\s*会\s*议[^\d]{0,8}?(\d{9,11})', s)
            if m:
                num = m.group(1)
            else:
...
# 模型会编造「张三/李四，北京大学/清华大学计算机系教授，长期从事量子计算…」家族假值
# （13 条实测污染，speaker 甚至被编成「约翰·史密斯」）。占位人名+名校/模板句组合才判
# 幻觉，避免误杀同名真人；仅用于校验 LLM/VLM 生成值，正文权威标签提取不经过此守卫。
# 2026-09-10 收紧：「李华」「李航」是常见真实中文人名（李华更是英语教材主角），
# 留在名单里只要真人讲者叫李华、值里又带个逗号，就会被误判幻觉而丢弃 → 按
# 「宁漏拦勿误杀」移出。保留项均为教材化占位名或明显虚构名。
_HALLUC_NAME_RE = re.compile(
    r'^(张三|李四|王五|约翰·史密斯|John\s*Smith)\s*[，,：:]')
...
def _apply_vlm_to_result(result, f, default_year, publish_time, title_year, url_year,
    ...
    # 多主讲人（VLM 常以「、」并列，如 xz65 第5场「龚雅云、黄嘉正」）：
    # 逐段过姓名校验，全过才有效；单人值退化为原有判定。
    _vlm_sp_segs = [x.strip() for x in re.split(r'[、,，/]', _vlm_speaker) if x.strip()]
    _vlm_speaker_valid = bool(_vlm_sp_segs) and all(
        _looks_like_real_name(x) for x in _vlm_sp_segs)
    _MAP = [
...
_NON_LECTURE_KW = [
    ...
    '纪实', '侧记', '花絮', '速递', '快讯',
    '展览', '作品展', '开幕', '拉开帷幕', '启动仪式', '项目启动', '活动启动', '启动会', '启动式',
...
def _normalize_label_text(text):
    ...
    labels = [
        ...
        # 时间地点人物
        '讲座地点', '讲座时间', '地点', '时间', '主讲人', '主讲师', '报告人', '讲座人', '主讲嘉宾', '讲座嘉宾', '演讲人', '主讲',
        '学术主持', '主办单位',
        # 线上会议平台标签（maths 等 CMS 把「腾 讯 会 议」拆成单字空格，须整体折叠后才能被 FB3 兜底识别）
        '腾讯会议',
        # 单位类复合标签：必须整体折叠，防「单位」裸词命中「组织单位」内部（physics787 病根）
        '承办单位', '协办单位', '组织单位', '支持单位', '指导单位',
        # 简介/摘要/内容
...
# ---------------------------------------------------------------------------
# 统一出口闸门（2026-09-05 全局诊断产物）
# 不管字段来自规则、模型A融合还是 VLM，入库前一律跑同一套确定性污染检测：
# 元信息块模板（题目：/报告人：/组织单位：…）、邀请语尾部、页脚标记、悬挂残段。
# 命中即截断；规则与模型共用同一标准——这才是「规则兜底」的真正含义：
# 不是规则先算一遍，而是规则标准在出口最后一道把关。
# ---------------------------------------------------------------------------
def _gate_record(r, url=''):
    ...
    for fld in ('abstract', 'speakerBio'):
        ...
        # 摘要开头 30 字已在简介中出现 → 摘要不是独立内容，置空（信息仍在简介里）。
        if fld == 'abstract' and nv:
            _bio_v = (r.get('speakerBio') or '').strip()
            if len(nv) >= 30 and len(_bio_v) >= 30 and nv[:30] in _bio_v:
                nv = ''
        if nv != v:
            # 截断后所剩无几且原值较长：整段多为污染块，置空比留残段诚实
            if len(nv) < 12 and len(v) >= 30:
                r[fld] = ''
                changed.append(fld + ':emptied')
            else:
                r[fld] = nv
                changed.append(fld)
    inv = (r.get('inviter') or '').strip()
    if inv:
        # 嵌套括号残段：「余虓) 中南大学」→「余虓」（右括号先于左括号出现，从右括号截断）
        _pos = inv.find('(')
        _end = inv.find(')')
        if _end != -1 and (_pos == -1 or _end < _pos):
            inv2 = inv[:_end].strip(' ,，、;；')
            if inv2 and inv2 != inv:
                r['inviter'] = inv2
                changed.append('inviter')
    if changed:
        r['qaGate'] = '|'.join(changed)
    return r
...
def apply_exit_gate(out, url=''):
    ...
    if isinstance(out, dict):
        return _gate_record(out, url)
    if isinstance(out, list):
        return [_gate_record(x, url) for x in out if isinstance(x, dict)]
    return out
...
def _parse_detail_impl(html, url, college, campus, default_year=None, list_title=None, skip_news_filter=False):
    ...
    # wxy 站把导航区包在 <article class="news-column">，find('article') 曾先命中导航（45字），
    # 真正正文 div.news-text（255字）轮不上 → body_text 只剩导航，时间/主讲全部落空
    # （wxy3238 抓成报名截止日期的根因）。过短才换候选，普通站点行为完全不变。
    _CONTAINER_SELECTORS = (
        ('div', 'wp_articlecontent'), ('div', 'wp_entry'), ('div', 'article-content'),
        ('div', 'container-left'), ('div', 'posts_content'), ('article', None),
        ('div', 'content'), ('div', 'news-details-all'), ('div', 'news-details-middle'),
        ('div', 'news-text'), ('div', 'entry-content'),
    )
    content_div = None
    _first = None
    _first_len = -1
    _best = None
    _best_len = -1
    for _tag, _cls in _CONTAINER_SELECTORS:
        for _el in soup.find_all(_tag, class_=_cls):
            _t = re.sub(r'\s+', ' ', _el.get_text(' ')).strip()
            if _first is None:
                _first, _first_len = _el, len(_t)
            if len(_t) > _best_len:
    ...
    # 连写多主讲人标记：报告人字段以「[头衔]姓名职称」拼接/分隔多位主讲人时置为
    # segments 列表（主分支逗号分隔检测与 OCR 连写分支共用，须在主分支前初始化）
    multi_speakers = None
    speaker_pat = (
    ...
    if m:
        ...
        if not _multi_name_matched:
            ...
            if len(_ms) >= 2 and all(_looks_like_real_name(n) for n in _ms):
                ...
                result['speakerSource'] = 'label'
                _multi_name_matched = True
        # 多主讲人（逗号/顿号分隔的「姓名职称，姓名」，如物理学院老页
        # 「讲座人：高兴森教授,张飞」——两位主讲人各报告一场，题目以 1)/2) 分场）。
        # 每段剥职称后须是 2-4 字纯中文姓名，防止「张三，华南师范大学教授」式
        # 姓名+单位被误拆（单位剥职称后超长/含非姓名字，过不了姓名校验）。
        if not _multi_name_matched:
            _segs2 = re.split(r'[，,、]', _sp_orig)
            if len(_segs2) >= 2:
                _pairs = []
                for _s2 in _segs2:
                    _s2 = _s2.strip()
                    ...
                    if re.fullmatch(r'[\u4e00-\u9fff·]{2,4}', _n2) and _looks_like_real_name(_n2):
                        ...
                        break
                if _pairs:
                    result['speaker'] = '、'.join(p['name'] for p in _pairs)
                    result['speakerSource'] = 'label'
                    if _pairs[0]['honorific'] and not result.get('speakerTitle'):
                        result['speakerTitle'] = _pairs[0]['honorific']
                    _multi_name_matched = True
                    multi_speakers = _pairs
        if not _multi_name_matched:
    ...
    # 单位类锚点必须用「组织单位/主办单位/…」复合标签整体（带冒号），裸「单位」曾命中
    # 「组织单位：」内部致截断错位、摘要留下「…。 组织」残尾（physics787 实测病根）。
    m = re.search(
        rf'{abs_pat}'
        r'([\s\S]+?)'
    ...
    if m:
        ...
        if len(abstract) > 5:
            ...
            # 邀请类尾部清理（词表统一，含 欢迎老师/欢迎同学/欢迎有兴趣 等变体）
            abstract = _fv.INVITATION_TAIL_RE.sub('', abstract).strip()
            result['abstract'] = abstract
    ...
    if result.get('speakerAffiliation'):
        ...
        if _TITLE_ONLY.fullmatch(_aff_dn):
            ...
            # 纯拉丁单位（"University of Oslo"）保留词间空格。
            if re.search(r'[\u4e00-\u9fff]', _aff2):
                result['speakerAffiliation'] = re.sub(
                    r'(?<=[\u4e00-\u9fff])\s+(?=[\u4e00-\u9fff])', '', _aff2).strip()
            else:
    ...
    if _abs:
        ...
        # 仅剔除「邀请类」尾部（词表统一：欢迎广大/欢迎各位/欢迎师生/欢迎老师/诚挚邀请/敬请…），
        # 不误伤正文合法的「受到欢迎」「广受欢迎」等表述（原 bare「欢迎」会截断摘要中部）。
        _abs = _fv.INVITATION_TAIL_RE.sub('', _abs).strip()
        if len(_abs) > 3:
    ...
    if sessions:
        ...
        for r in split_recs:
            ...
            # skip_news_filter 一致性豁免（2026-09-25，用户授权）：该开关的设立本意就是
            # 「该来源整栏为讲座预告、发布常晚于讲座时间」（整页短路已豁免于 5237 行）。
            # 若拆分后逐条仍按 publishTime > lectureStart 判回顾稿，则同一来源、同一页面
            # 仅因「能否识别出多场」而得到截然相反的结论——不拆则入库 1 条、拆出后全被
            # 判回顾而整页返回 None（em 3705/6287 实测：发布 11:04/16:12 晚于讲座 09:00，
            # 拆出的 2 场全丢，反而比不拆更差）。故来源已显式跳过新闻过滤时，拆分后的
            # 逐条判定同样豁免，保持整页与逐条口径一致。未标记来源行为完全不变。
            if not skip_news_filter and is_news_record(r, poster_page=poster_only):
...
def _cn_num_to_int(s):
    """中文数字（一~百）转 int，用于场次编号比较；非中文数字原样返回。"""
    _D = {'零': 0, '一': 1, '二': 2, '三': 3, '四': 4, '五': 5,
          '六': 6, '七': 7, '八': 8, '九': 9}
    s = (s or '').strip()
    if not s:
        return None
...
def _detect_nth_field_sessions(text, default_year=None, publish_time=None,
                               title_year=None, url_year=None, title='',
                               base_start=None, base_end=None):
    """候选9（兜底，em 经管等）：「第N场/讲/期」后紧跟**字段标签块**的系列页。

    正文形如（em/3705）：
      第一场题目:Are Managers Allied… 主讲人:钟宁桦副教授同济大学 时间:2013年12月20日(周五)上午9:00-12:00 地点:文二栋五楼会议室
      第二场题目:agglomeration and markup 主讲人:陆毅教授新加坡国立大学 时间:… 地点:…
    或（em/6287，marker 与标签间有空格/冒号）：
      第一场 : 题目: 城市病 : 控人还是治病主讲人: 陆铭教授( 上海交通大学特聘教授 ) 时间:… 地点:…

    与候选5 的分工：候选5 的 marker 带负向预查 (?![题主报人目介摘])，会**直接拦掉**
    「第一场题目:」（marker 后紧邻「题」字），且其 topic 只取冒号后 3-40 字，
    长英文题目（85 字符）超限失败。本候选反过来把「marker 紧邻字段标签」当作
    **强特征**——要求块内同时含「题目」与「主讲人/报告人」字段标签，误命中风险低。

    守卫（对齐候选5）：① marker 号须有 ≥2 个不出现在 title 中的不同号（MS5-GUARD，
    排除面包屑/上一篇重复）；② 段内须含时钟；③ 段内须同时含题目与主讲人标签。
    """
    _NTH_RE = re.compile(r'第\s*([一二三四五六七八九十百零两0-9]+)\s*(?:场|讲|期)')
    _TOPIC_FIELD = re.compile(r'题\s*目\s*[:：]')
    _SPK_FIELD = re.compile(r'(?:主讲人|报告人|演讲人)\s*[:：]')
    _NEXT_FIELD = re.compile(
        r'(?:主讲人|报告人|演讲人|主持人|时间|地点|摘要|简介|内容|评论人)\s*[:：]')
    _HAS_CLOCK = re.compile(r'\d{1,2}\s*[:：点]\s*\d{1,2}')

    marks = list(_NTH_RE.finditer(text))
    if len(marks) < 2:
        return []
    _nums_in_title = {m.group(1) for m in _NTH_RE.finditer(title or '')}
    # 只保留号不在 title 中出现的 marker（去掉面包屑/「上一篇」的模板重复）
    marks = [m for m in marks if m.group(1) not in _nums_in_title]
    if len(marks) < 2:
        return []
    _distinct = {m.group(1) for m in marks}
    if len(_distinct) < 2:
        return []
    ...
    for i, mk in enumerate(marks):
        ...
        if not (_TOPIC_FIELD.search(seg) and _SPK_FIELD.search(seg)):
            continue
        if not _HAS_CLOCK.search(seg):
            continue
        tm = _TOPIC_FIELD.search(seg)
        rest = seg[tm.end():]
        nm = _NEXT_FIELD.search(rest)
        topic = (rest[:nm.start()] if nm else rest).strip()
        topic = _clean_session_topic(topic)
        if not topic or len(topic) < 2:
            continue
        dt = parse_cn_time(_page_date_str + seg, default_year=default_year,
                           publish_time=publish_time,
                           title_year=title_year, url_year=url_year)
        if not dt or not dt.get('start'):
            continue
        # _numbered=True：显式「第N场」编号 + 各自字段块属**编号型多场**，与候选4
        # （题目N：）同性质，豁免末尾 distinct-time 检查——em 系列页常把两场写成同一
        # 页眉时段（3705 两场均「上午9:00-12:00」），若要求时间互异会把真实多场误并。
        # 本候选的块结构规整（题目/主讲人/时间/地点），可在此直接定出主讲人与单位：
        # 通用逐块路径（split_record_by_sessions）为取姓名会先把职称连同其后的单位一起
        # 删掉（cand_core 的 `(职称).*$`），导致「钟宁桦副教授同济大学」取不到单位而
        # 继承整页值、把第二场的「新加坡国立大学」串到首场（em/3705 实测）。
        # 该通用路径牵一发动全身（改宽会让 psy961/cs5708 等页取到错误单位），故不改它，
        # 改由本候选按块内规整字段显式给出——影响面仅限本候选命中的页。
        _spk_m = _SPK_FIELD.search(seg)
        _spk, _aff = None, None
        if _spk_m:
            _srest = seg[_spk_m.end():]
            _cut = re.search(r'(?:时间|地点|摘要|简介|主持人)\s*[:：]', _srest)
            if _cut:
            ...
            for _l in (3, 2):
                if len(_head) < _l:
                    continue
                _m = re.match(rf'^([\u4e00-\u9fff]{{{_l}}})', _head)
                if _m and _looks_like_real_name(_m.group(1)):
                    _nm = _m.group(1)
                    break
            if _nm:
                _spk = _nm
                # 单位从职称起算（_extract_affiliation 能处理「教授新加坡国立大学」）
                _tail = _srest[_srest.find(_nm) + len(_nm):].strip(' （(，,）)')
                _a = _extract_affiliation(_tail)
                if _a:
                    _aff = _a
        if _spk:
            cand.append({'topic': topic, 'start': dt['start'], 'end': dt.get('end'),
                         'block': seg, '_no': mk.group(), '_numbered': True,
                         'speaker': _spk, 'affiliation': _aff or '',
                         'splitMode': 'nth-field'})
            continue
        cand.append({'topic': topic, 'start': dt['start'], 'end': dt.get('end'),
                     'block': seg, '_no': mk.group(), '_numbered': True,
    ...
    if len(cand) < 2:
        return []
    # 同号重复块去重（面包屑与正文可能各命中一次）
    _seen_no = {}
    out = []
    for s in cand:
        _no = _NTH_RE.search(s['_no'])
        _k = _cn_num_to_int(_no.group(1)) if _no else s['topic']
        if _k in _seen_no:
            continue
        _seen_no[_k] = s
        out.append(s)
...
def _agenda_slot_speaker(seg):
    """议程时段行后的姓名串：取冒号前文本，按 、,， 拆开逐个判人名。

    支持「刘愿、张磊:星星之火…」「李增福、曾林:…」这类多人合报；
    任一人名通过 _looks_like_real_name 即认为该行是报告行（姓名交给下游按块提取）。
    """
    mc = re.match(r'^\s*([^:：\d]{2,16}?)\s*[:：]', seg)
    if not mc:
        return None
    ...
    for part in re.split(r'[、,，/]|和|与', raw):
        part = part.strip()
        if 2 <= len(part) <= 4 and _looks_like_real_name(part):
            return raw
    return None
...
def _detect_agenda_slot_sessions(text, default_year=None, publish_time=None,
    ...
    def _mins(hhmm):
        m = re.match(r'\s*(\d{1,2})\s*[:：]\s*(\d{2})', hhmm)
    ...
    cand = []
    for i, mk in enumerate(marks):
        a, b = _mins(mk.group('s')), _mins(mk.group('e'))
        ...
        if not dt or not dt.get('start'):
            continue
        # 'speaker' 键会被 split_record_by_sessions 直接采用（议程表无「主讲人:」标签，
...
def _split_english_speaker(sp):
    ...
    # "Prof." 缩写 + 无空格粘连（"Prof.Takaaki Nomura"，iqm447 实测）也须剥离：
    # 允许头衔与姓名之间零空格，但剥离后须紧跟大写字母（防误剥 "Profanity" 类词）。
    _hm = re.match(r'^(Associate\s+Professor|Full\s+Professor|Professor|Prof\.?|'
                   r'Ph\.D\.?|PhD|Dr\.?|Doctor)\s*(?=[A-Z])', s, flags=re.I)
    if _hm:
        ...
        if re.fullmatch(r'Dr\.?', _raw, re.I):
            ...
            _title = 'Ph.D.'
        elif re.fullmatch(r'Prof\.?', _raw, re.I):
            _title = 'Professor'
        else:
            _title = _raw
        s = s[_hm.end():]
...
def _split_topic_segments(topic, n):
    """把「1)A 2)B」式多场题目拆成 n 段；段数不等于 n 时返回 None。
...
def _pair_speakers_to_segments(speakers, segs):
    """把中文主讲人按拼音与各段尾部 "By <拉丁名>" 配对（physics791 场景）。

    页面「讲座人：高兴森教授,张飞」的顺序与题目 1)/2) 的 By 顺序可能交叉
    （1) By Ms Fei Zhang ↔ 张飞，2) By Xingsen Gao ↔ 高兴森），按顺序配对会错。
    以拼音字符多重集相等为判据（zhangfei ↔ feizhang、gaoxingsen ↔ xingsengao），
    且要求每位主讲人恰好唯一配对（双射）；任一环节失败返回 None（调用方保持
    共享题目——宁可共享不可错配）。
    """
    try:
    ...
    for s in segs:
        m = re.search(
            r'\b(?:By|BY)\s+(?:(?:Ms|Mr|Mrs|Dr|Prof|Professor)\.?\s+)?'
            r'([A-Za-z][A-Za-z\.\-]+(?:\s+[A-Za-z][A-Za-z\.\-]+)*)\s*$', s.strip())
        if not m:
            return None
        lat.append(re.sub(r'[\s\.\-]+', '', m.group(1)).lower())
    if len(lat) != len(speakers):
        return None
    assign = {}
    used = set()
    for si, spk in enumerate(speakers):
        py = re.sub(r'[\s\.\-]+', '', ''.join(lazy_pinyin(spk.get('name') or ''))).lower()
        if not py:
            return None
        cands = [ti for ti, l in enumerate(lat) if sorted(l) == sorted(py) and ti not in used]
        if len(cands) != 1:
            return None
        assign[si] = cands[0]
        used.add(cands[0])
...
def _split_by_speakers(base, speakers):
    ...
    for i, spk in enumerate(speakers):
        ...
        rec['notes'] = []
        if seg_assign is not None:
            _seg = re.sub(r'\s*(?:[Bb]y\s+[A-Za-z][A-Za-z\.\-\s]*)$', '',
                          segs[seg_assign[i]]).strip(' ；;，,')
            if _seg:
                rec['topic'] = _seg
                rec['topicPairedBy'] = 'pinyin'
        out.append(rec)
    return out
...
def _detect_multi_session_impl(text, title='', default_year=None, publish_time=None,
    ...
    if len(_bm) >= 2:
        # 聚类守卫：书名号并列必须"相邻成组"——相邻两对《》之间仅隔姓名（≤12 字）
        # 且只由汉字/空格组成（无标点括号）。散落在简介散文里的书名号（psy1136 bio
        # 「经管杂志《财富》」与「…报告《中国高级经理人…》」之间隔着括号/逗号/引号）
        # 不构成并列场次，曾误拆出『管杂志』『康报告』这类姓名碎片。
        # 只保留最大成组簇，不足 2 对则放弃该候选。
        _clusters = [[_bm[0]]]
        for _prev, _nxt in zip(_bm, _bm[1:]):
            _gap = text[_prev.end():_nxt.start()]
            if len(_gap) <= 12 and re.fullmatch(r'[\u4e00-\u9fff\s]*', _gap):
                _clusters[-1].append(_nxt)
            else:
                _clusters.append([_nxt])
        _best_cluster = max(_clusters, key=len)
        _bm = _best_cluster if len(_best_cluster) >= 2 else []
    if len(_bm) >= 2:
        _bs = base_start
        if _bs is None:
    ...
    if sessions_raw:
        ...
        if sessions_raw and (any(r['_has_clock'] for r in sessions_raw)
            ...
            # 各场题目互不相同。
            _norm_topics = [re.sub(r'\s+', '', r['topic']) for r in sessions_raw]
            if len(set(_norm_topics)) < len(_norm_topics):
                sessions = []
            else:
    ...
    if len(sessions) < 2:
        cand9 = _detect_nth_field_sessions(
            text, default_year=default_year, publish_time=publish_time,
    ...
    if len(sessions) < 2:
        cand10 = _detect_agenda_slot_sessions(
            text, default_year=default_year, publish_time=publish_time,
            title_year=title_year, url_year=url_year, title=title,
            base_start=base_start, base_end=base_end)
    ...
    # 编号型多场二次判定（2026-09-25，em 系列页）：候选9 位于瀑布末，常因上游候选已
    # 产出 ≥2 场而无机会执行（如 em/3705 由候选1 按「题目:」分块先命中），但这些场
    # 次确为显式「第N场/讲/期」编号型多场，源页常把各场写成同一页眉时段（3705 两场
    ...
    if not all(s.get('_numbered') or s.get('_same_time_ok') for s in sessions):
        ...
        if len(_b_nums) >= 2 and len(_b_nums) >= len(sessions):
            # 优先换用候选9 的场次：其 block 以「第N场」marker 为边界，字段更完整。
            # 上游候选1 按「题目:」分块时，块尾会拖到下一场 marker 为止
            # （em/3705 首场 block = 「…地点:文二栋五楼会议室第二场」），导致地点值
            # 混入下一场编号残片。候选9 无机会执行正因上游已占用瀑布，故在此补位。
            _c9 = _detect_nth_field_sessions(
                text, default_year=default_year, publish_time=publish_time,
                title_year=title_year, url_year=url_year, title=title,
                base_start=base_start, base_end=base_end)
            if len(_c9) >= 2:
    ...
    for i, s in enumerate(sessions):
        ...
        if sp_m:
            if s.get('speaker'):
                ...
                # 候选9 显式给出单位时一并使用：块内规整字段（主讲人:姓名职称单位）由候选
                # 直接解析，避免通用路径删职称时把单位一并删掉导致继承整页值串场。
                if s.get('affiliation'):
                    rec['speakerAffiliation'] = s['affiliation']
            else:
        ...
        if _abs_m:
            ...
            if len(_a) > 5:
                ...
                if ('»' in _a or '首页' in _a or '当前位置' in _a):
                    ...
                    _a = _fv.INVITATION_TAIL_RE.sub('', _a).strip()
                if len(_a) > 5:
                    rec['abstract'] = _a
        if _bio_map:
