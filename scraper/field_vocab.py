...
#    「主任医师」被「主任」截成「医师」。
ORG_TITLE_SUFFIXES = (
    # 复合词在前（_alt 仍按长度降序，此处为可读性）。
    '副总编辑', '副主任', '副校长', '副会长', '副秘书长', '副理事长', '总经理',
    '主任医师', '副主任医师', '总编辑', '常务理事',
    '主任', '处长', '院长', '所长', '科长', '局长', '部长', '总监',
    '经理', '工程师', '书记', '主席', '顾问', '会长',
    # 2026-09-26 审计收敛（G4）：以下各词原散落在 parsers._TITLE_ONLY /
    # parsers._clean_aff_token._ROLE_TAIL / audit_data_quality.BIO_TITLE_ONLY_RE
    # 三张手抄副本中、与主表分叉（「秘书长」单位这边放行、hybrid 判脏），
    # 统一并入主表，各副本改为本表派生。
    '秘书长', '理事长', '理事', '监事', '主编', '编辑', '记者',
    '经济学家', '经济学者', '学者', '专家',
)
