...
当详情页是**论坛/大会/研讨会**等含多场次议程的活动时，不拆分为多条讲座记录，而是作为一条整体记录提取。

**FC1 — 论坛/大会识别**（命中则跳过 MS1–MS5 多讲座拆分）：

满足任一条件即判定为论坛/大会：
```
条件A：标题含「论坛/大会/研讨会/峰会/学术会议/学术研讨会」之一
       且正文含「议程/日程/安排」+ 多场次标记（第X场/Session/场次）
条件B：正文含「论坛主题/大会主题/会议主题」标签
       且正文含「开幕式/闭幕式」之一
```
命中后标记 `forumConference = true`，采用 FC2 整体提取策略。

**FC2 — 整体字段映射**（论坛专用，独立于 F1–F6）：

| 字段 | 提取规则 | 示例 |
|------|----------|------|
| `title` | 原标题 | `"科玄论战"与百年人生观建设高端论坛` |
| `topic` | `论坛主题`/`大会主题`/`会议主题` 标签值 | 4 个主题方向 |
| `lectureStart` | 议程中**开幕式开始时间**；无开幕式则取首场开始时间。日期从 `论坛日期`/`大会日期`/`会议日期` 标签取，走 R4 补年 | `2024-09-28 08:30:00` |
| `location` | `论坛地点`/`大会地点`/`会议地点` 标签值 | `丽柏酒店八楼会议厅` |
| `abstract` | `论坛缘起`/`大会简介`/`会议简介`/`论坛简介`/`大会背景`/`会议背景`/`活动背景`/`活动简介` 标签值 | 论坛缘起全文 |
| `speaker` | `null`（论坛无单一主讲人） | — |
| `organizer` | `主办单位`/`承办单位` 标签值（新增可选字段） | 华南师范大学文学院、… |
| `has_time` | 开幕式/首场有具体时间则 `true` | `true` |

**议程时间解析**：
```
扫描议程文本所有时间模式：
  开幕式（8:30—9:00）→ start=8:30
  第一场 9:10—10:40 → start=9:10
  ……
lectureStart = 所有场次中最早的 start
（不设 lectureEnd，只取开幕式/首场开始时间）
```

**FC3 — RT2d 预告例外**（审签链误判修复）：

```
if RT2d 命中（页脚含「初审|…复审|…终审」审签链）:
    if publishTime 已定位 且 lectureStart 已定位:
        if publish_date ≤ lecture_date:
            → 不标记 retrospective（预告页的编辑流程产物）
            → notes 追加 "含审签链但发布早于活动日期，判定为预告"
        else:
            → 标记 retrospective（回顾稿，审签链确认）
    else:
        → 维持 RT2d 命中（时间未定位无法判定）
```

**执行顺序**：FC1 在 MS1 之前执行；FC1 命中 → FC2 提取 → FC3 修正 → 跳过 MS1–MS5。

---

### 1.6 多讲座拆分（MS1–MS5，仅 FC1 未命中时执行）

> 当一个详情页包含 ≥ 2 个独立讲座（不同题目/不同时间/不同主讲人）时，
> 拆分为 N 条独立记录。**论坛/大会页面由 FC1 接管，不走此流程。**

**MS1 — 多讲座检测**：

分块锚点（满足任一即触发分块扫描）：
- **模式 A（编号前缀）**：正文含 `(报告|讲座|报告人) (\d+) (题目|时间|地点)` 编号模式（如 `报告 1 题目`、`报告人 2`）
- **模式 B（重复标签）**：`主题：`/`题目：`/`Topic:`/`Title:` 出现 ≥ 2 次
- **模式 C（英文 TalkN，候选5b）**：`Talk1:` / `Talk2：` 分场标记（seri23 环境研究院双报告）。topic 取标记至「报告人/主讲人/欢迎」；页眉共享报告人/地点/时间（各场时间相同属正常），session 带 `_numbered=True` 豁免「时间互异」守卫。

期次锚点（辅助验证）：正则 `第[一二三四五六七八九十百千\d]+期` 统计期次数。
- 期次数 > 块数 → 可能双系列号（如 8 个期次 / 4 个主题块），以块数为准
- 期次数 < 块数 → 分块可能有问题，降置信度

块内完整性检查：每个块须同时含可解析日期 + 可解析时间/时段，否则标记 invalid。

**多讲座判定**：valid 块数 ≥ 2 **且** 这些块的日期/时段至少有 2 个不同。

**MS2 — 跨行日期时间关联**：

每个块合并换行为单行（保留空格分隔），再交给 R1+R4+R6 解析。
跨行文本 `2021年11月17日\n下午2:00-4:00` 合并后可正常解析。

**MS3 — 防误判**（命中任一则不拆分）：

1. **单期讲座**：valid 块数 < 2
2. **同主题多时段**：所有 valid 块 topic 完全相同 → 视为同讲座多场次，取首场作 lectureStart
3. **列表页列举**：≥ 50% 的块含独立详情页链接（指向不同 URL）→ 返回链接供分别抓取
