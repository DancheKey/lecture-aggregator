...
parsers.parse_detail(html, url, college, campus)
   ...
   │  ├─ 日程表就地替换 _replace_schedule_tables_with_text（真表头扫描→「主题：/主讲人：/时间：」行）
   ...
   │  ├─ 多场拆分：候选瀑布 0-11（含质量闸门 _ms_gate_filter / 去重守卫 _ms_dedup_guard，
   │  │   见 §1.6；MS5 逐条回顾剔除后重算 index/count）
   │  ├─ LLM 融合：hybrid.apply_llm_text_hybrid（模型A 仅填空）+ llm_provider + cited_judge 引证裁决
   │  └─ 新闻过滤 is_news_record + is_news_article（两层）+ 出口闸门 _gate_record（§1.8）
   ▼
data/lectures.json  { updatedAt, data:[...] }     ← 爬虫唯一数据源
   │  scripts/generate_frontend_data.py 加工（排除名单过滤 + unitType 标注 + speakerKeys）
   │  ⚠️ 不是 cp 副本：直接 cp 会让排除名单失效、unitType 丢失（2026-08-05 体检修正）
   ▼
site/lectures.json  +  site/lectures/{latest,stats,chunks}.json + chunk_NNNN.json
   │
   ▼
前端 site/index.html + app.js（公网按 chunks.json 分片加载；本地 /api/lectures 读全量）
```
...
- **地点兜底 FB4（议程括号会场）**：标签地点为空时，从议程括号提取会场（≤20 字、含 楼/栋/室/厅/教室/会议室/报告厅/演讲厅，排除 主持人/发言嘉宾/线上/注）。ggy5326「会议签到（文3栋一楼108门前）」「（一楼第二演讲厅）」；页脚「地址：广东省广州市番禺区…」仍由 FB1 行政区守卫拒绝。
- **多讲者清洗守卫分隔符**：F3 主讲人清洗守卫（label 路径 / 终检）与 VLM 出口守卫统一按 `[、,，]` 分段校验——半角逗号多讲者串不再整串判非人名而清空，各段均为有效人名即保留。
...
### 1.5 论坛/大会页面（⚠️ 旧 FC1–FC3 设计已废弃，从未实现）

> **2026-09-26 勘误**：本节旧版本描述的「FC1 论坛识别 → FC2 整体字段映射 → FC3 RT2d
> 例外，产出 `forumConference=true`」是一份**从未落地的设计稿**——全仓 Python 代码中
> `forumConference` 0 命中（仅主库残留过 1 条历史记录带该键，已于 2026-09-26 清理）。
> 论坛/大会/研讨会页面**不走任何专用通道**，与普通详情页一样进入 §1.6 的候选瀑布，
> 由其中面向议程格式的候选负责拆分。

论坛/大会页的实际处理路径（按页面议程格式分工）：

| 议程形态 | 处理者 | 实证页 |
|---|---|---|
| HTML `<table>` 日程表（真表头：时间/主讲人/题目/发言人…） | `_replace_schedule_tables_with_text` 就地转「主题：/主讲人：/时间：」行 → 候选1 拆分 | em/3769、em/5983 分组讨论 |
| 议程行「时段 名:题目」（含多人合报） | 候选10 V1 | em/7636 |
| 议程行「时段 名(单位) 题目：X」 | 候选10 V2（时间-人名-题目按场对齐） | em/7264、em/7542 |
| 行内「题目/主题演讲N: X 演讲专家: 名(单位)」 | 候选10 V4 | em/5949 |
| 编号行「N. 名(单位) 题目：X」（各场无独立时刻，共享环节窗） | 候选11 numbered-agenda | em/5983 上午主报告 |
| 「主题：X 主讲人：Y」展平行（无时钟/仅时长） | 候选1 多人异名兜底（label-speakers） | em/3771、em/4169 |

一个议程页往往混多种形态，且表格在解析早期已被替换为文本——**不要试图按"学院"或
"会议类型"写专用规则**（2026-09-26 审计结论：统一的方向是扩"页面格式"覆盖 + 瀑布
质量闸门，见 §1.6 末尾）。无法自动覆盖的混合结构页（如 em/5983、em/5949）按 §3.1
流程人工拆分落库（splitMode='manual'，增量合并基底锁定保护）。

---

### 1.6 多场拆分：候选瀑布 + 质量闸门（权威描述，对齐 `parsers._detect_multi_session_impl`）

> 当一个详情页包含 ≥ 2 个独立讲座（不同题目/不同时间/不同主讲人）时，
> 拆分为 N 条独立记录。论坛/大会页**没有专用通道**，同样走本瀑布（§1.5）。

**候选瀑布（2026-09-26 现状，候选 0→11，严格顺序、仅当上游不足 2 场时下游兜底；
不引入评分机制——Q1 决策，评分更难调试且易隐性回归）**：

| 候选 | 分场锚点 | 代表页 | 备注 |
|---|---|---|---|
| 0 | 段落内并列「第一讲：A 第二讲：B」 | wxy/2979 | |
| 0b | 书名号并列「莫逊男《题目A》 胡小勇《题目B》」 | 汕尾教学论坛 | 姓名+成组守卫 |
| 1 | 「题目/主题：」重复标签分块 | psy 双讲页 | 含页眉日期窄口径回填（仅限块内自带「时间：时钟」标签）、多人异名兜底 |
| 2 | 离散「报告N/讲座N」 | cs 源 | |
| 3 | ≥2 个「报告时间:」字段锚点 | cs/5400 | |
| 4 | 「题目N：/报告题目N：」编号 | cs/4268 | `_numbered` 豁免时间互异 |
| 5 | 「第N讲/场/期」 | abdn 系列 | MS5-GUARD：号全在 title 中→模板重复清空 |
| 5b | 英文「TalkN:」 | seri/23 | `_numbered` |
| 6 | 裸「专题N：」并列 | ctld 通识课 | 页眉时间兜底，`_numbered` |
| 7 | 「1. 题目 时间：…」纯编号列表 | ibc/2779 | |
| 8 | 「报告人N：」编号 | cs/1932 | |
| 9 | 「第N场」+ 题目/主讲人字段块 | em/3705/6287 | `_numbered`；瀑布末有编号型二次判定补位 |
| 10 | 议程「时段+报告行」三形态 V1/V2/V4 | em/7636/7264/5949 | 三重守卫：时长≤60min、行解出姓名+题目、噪声词 |
| 11 | 编号行「N. 名(单位) 题目：X」 | em/5983 | 时间取前方最近环节窗；人名互异守卫 |

**质量闸门 `_ms_gate_filter`（2026-09-26，接入全部 14 个接受点）**：候选产出剔除
伪场次（时钟/时段+后勤词、报到/会议报到等脚手架行、尾部开括号残片）后不足 2 场、
真场次主题不足 2 个互异、或跨场「同 (月,日) 不同年」（日期取错年的实证特征，
ctld/1280）→ 该候选整体放弃、落至下一候选。**动机**：劣质候选曾抢占瀑布使下游
优质候选永无机会（em/5983 被 repeated-label 垃圾占位）。

**去重守卫 `_ms_dedup_guard`**：同 (topic,start) 去重（保留信息更完整块）、
导航链/行政通知行（『…的通知』结尾）剔除、全部同主题且同日历日→不拆（MS3-2）、
讲座类链接占比过高→判列表页不拆（MS3-3：讲座类链接数 ≥ max(场次数,5)，
不返回链接）、时间互异检查——`_numbered`/`_same_time_ok`（多人异名共享页眉
时间，em/3771/4169）豁免；「^关于」前缀过滤已收窄为「的通知」结尾
（曾误杀顾乃华《关于广东产业高质量发展的几点思考》并致编号错位）。

**块内完整性（MS1 修订）**：仅当「全部候选块都无时钟 且 日期全部相同」才视为
共享页日期不拆分；日期互异（≥2 个不同日期）即使无时钟也保留为独立场次（ai/163）。
旧文档的「期次锚点统计/降置信度」机制从未实现，已删除。
...
| `title` | **原标题保持不变，不拼 topic** | 铁律：`test_invariants` 锁定 title==clean(listTitle)；旧文档「原标题+`---`+主题」与代码相反，系列位次由 lectureIndex/前端「（第N场）」后缀表达 |
| `host` | 该块 `主持人：`标签值 | 独立（可选字段） |
| `location`（线上会议） | 该块「会议号/会议 ID：」；缺则全文「腾讯会议专题X:ID」按序匹配 | 独立；格式「平台 号码」，并入 `location`（2026-09-23） |
| `speaker` | 该块 `主讲人：`；缺失从前序块继承，标注 `speakerSource='inherited'` | 逐块优先；候选9/10/11 显式给出的 speaker/affiliation 直接采用 |
...
若有 `参与者：` 标签，提取为 participants 可选字段。

**多主讲人连写拆分（独立于 MS 块拆分的路径，与 MS 互斥）**：同一公告 speaker 为
「A, B / A、B」串、整串非人名、各段互异且均为有效人名时拆为 N 条（xz301 双讲者）：
- 题目拆段：「1)A 2)B」编号优先；失败且 n=2 时按首个左右均 ≥4 字的「和」拆
  （`集成光子学和超快光学技术和应用` → `集成光子学` / `超快光学技术和应用`）。
  topic 非空但拆不出 n 段（VLM 把系列名填进 topic）时回退 title 再拆；触发派生
  与 `_split_by_speakers` 取段均遵循此回退。
- 段↔人配对：先按段尾 `By <拉丁名>` 与中文名拼音双射；失败则顺序配对
  （黄加耀↔第1段、刘轩奕↔第2段）。
- 块内 `报告人：<单位>` 而姓名在后续拉丁行时（seri23「报告人: 西班牙研究理事会
  环境化学研究所 / Silvia Lacorte教授」），凡 cand 含单位关键词即优先提取英文
  姓名+行内职称，防单位前 2~3 字被当中文人名（「西班牙」）。
- VLM 多场返回且各场 topic 相同/为空时，按 title 拆段逐场分配；各场 topic 互异
  则不动。
...
拆分后的 N 条记录每条独立过 RT0–RT2。某条被标记 retrospective 则单独排除，不影响其他条。
`skip_news_filter` 来源（整栏预告、发布常晚于讲座时间）逐条豁免（em 3705/6287 实测口径）。
**2026-09-26 幽灵计数修复**：剔除后剩余 M(<N) 条按原编号顺序重排 `lectureIndex=1..M`、
`lectureCount=M`；仅剩 1 条时视为单场（isMultiLecture=False、编号清空——系列位次
保留在 title/topic 文本）。存量 11 页由 `scripts/fix_ghost_counts.py` 清洗
（xz/252 标 5/5 仅存 1 条、ai/163 编号 3-6 共 4 条等）。
...
3. **出口统一闸门**：`parsers.parse_detail` 包装层在返回前对每条记录的
   ...
   （「…。 组织」）命中即截断，截后 <12 字且原值 ≥30 字则整段置空；**topic 另有
   专项边界截断**（拆分场次的 topic 常拖「报告专家:…北京师范大学」类串，见
   `parsers._gate_record`）。规则与模型共用同一标准；**"规则兜底"的真正含义是
   规则标准在出口最后一道把关，而非规则先算一遍**。
...
- **同单位命中**（A、B 同学院）：同样 `merge-into-A` 融合（C 方案定稿，2026-09-23）。信息零丢失，且 B 的源链接保留在 A.sources，无需纠结留新留旧。
  - 例外：**B 含轮次标记（第二轮/补充通知/更新等）→ 多轮替换，保留最新一轮**（由 `_has_round_marker` / `_strip_round_marker` 识别，优先级高于融合，避免把多轮通知并成一条）。
  - 同单位融合的前置门槛：与跨单位共用 `incremental_merge` 的命中判定（同主讲同日 + 标题相似度≥0.25，或同主讲同日 + 同精确时刻 + 同单位兜底）。库内「同单位同主讲同日但题目不同」= 同日多场不同讲座（如李红岩上午/下午各一场），相似度命不中、自动保留多场，**不会被误合**。
- **无主讲人大会 / 多轮通知命中**（speaker 留空，如 ggy5326 类研讨会）：同样 `merge-into-A` 融合。这类记录不进主讲人索引，此前完全检测不到、只能依赖 `dedup_existing.py` 事后捞；上线后无法自动处理，故新增 `no_speaker_index`（键 `(college, date)`，收录 speaker 留空且有单位的记录）覆盖。判据更严（无主讲人弱信号）：同单位 + 标题/题目相似度 ≥ 0.6 + 排除「双方都含精确时刻且时刻不同」（同日不同场次）。此举解决用户核心诉求——同单位大会多轮通知（无轮次标记、可能无精确时刻）上线后自动融合，不靠翻旧库。
- **增量 vs 全量判定差异**：增量检测**不靠 location 兜底层**（立场：location unreliable）；全量 `scripts/dedup_existing.py` 保留「同 location + 双方单位缺失 + 标题相似度」兜底路径。两者判定能力不一致，已知且接受——`dedup_existing.py` 仍作为事后全量清理兜底（尤其跨单位无主讲人大会等增量未覆盖情形）。
